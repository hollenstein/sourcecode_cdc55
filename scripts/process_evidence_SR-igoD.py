...
# Process the MaxQuant results of the stress response igo1/2 delta experiment
if __name__ == '__main__':
    root = '../'
    table_dir = os.path.join(root, 'tables')
    fasta_path = os.path.join(root, 'yeast_cont_20140324.fasta')
    mq_txt_folder = 'MQ_txt_SR-igoD'
    evidence_path = os.path.join(
        root, 'MaxQuant', mq_txt_folder, 'evidence.txt'
    )
    experiments_toreverse = ['01_osmo_igo', '02_osmo_igo', '03_osmo_igo']
    setup = 'SR-igoD'
