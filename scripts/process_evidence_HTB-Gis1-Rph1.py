...
# Process the MaxQuant results of the Gis1 and Rph1 HTB purification experiments
if __name__ == '__main__':
    root = '../'
    table_dir = os.path.join(root, 'tables')
    fasta_path = os.path.join(root, 'yeast_cont_20140324.fasta')
    mq_txt_folder = 'MQ_txt_HTB-Gis1-Rph1'
    ...
    evidence['Setup'] = evidence['Experiment'].apply(lambda e: e.split('_')[0])
    experiments_toreverse = np.unique(evidence['Experiment'])
    MQ.reverse_ratios(evidence, experiments_toreverse)
    MQ.normalize_protein_abundance(evidence, 'GIS1')
    MQ.normalize_protein_abundance(evidence, 'RPH1')
